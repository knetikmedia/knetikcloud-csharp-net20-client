...
path = path.Replace("{" + "id" + "}", ApiClient.ParameterToString(id));
        ...
        public PaymentMethodTypeResource GetPaymentMethodType (int? id)
        {
            ...
            // verify the required parameter 'id' is set
            if (id == null) throw new ApiException(400, "Missing required parameter 'id' when calling GetPaymentMethodType");
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling GetPaymentMethodType: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling GetPaymentMethodType: " + response.ErrorMessage, response.ErrorMessage);
    
            return (PaymentMethodTypeResource) ApiClient.Deserialize(response.Content, typeof(PaymentMethodTypeResource), response.Headers);
        }
 ...
 if (order != null) queryParams.Add("order", ApiClient.ParameterToString(order)); // query parameter
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling GetPaymentMethodTypes: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling GetPaymentMethodTypes: " + response.ErrorMessage, response.ErrorMessage);
    
            return (PageResourcePaymentMethodTypeResource) ApiClient.Deserialize(response.Content, typeof(PageResourcePaymentMethodTypeResource), response.Headers);
        }

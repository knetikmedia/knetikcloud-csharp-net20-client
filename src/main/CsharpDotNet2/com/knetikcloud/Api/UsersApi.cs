...
namespace com.knetikcloud.Api
{
 ...
 if (filterLastActivity != null) queryParams.Add("filter_last_activity", ApiClient.ParameterToString(filterLastActivity)); // query parameter
 if (filterIdList != null) queryParams.Add("filter_id_list", ApiClient.ParameterToString(filterIdList)); // query parameter
...
path = path.Replace("{" + "tag" + "}", ApiClient.ParameterToString(tag));
        ...
        public void SubmitPasswordReset (PasswordResetRequest passwordReset)
        {
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling SubmitPasswordReset: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling SubmitPasswordReset: " + response.ErrorMessage, response.ErrorMessage);
